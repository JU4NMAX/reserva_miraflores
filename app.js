/**
 * IECM - Visualizador 3D Sistema de Monitoreo Hídrico v3.2
 * Con vista explosionada y modo simulador en Reserva Miraflores
 */
...
class Visualizer3D {
  constructor() {
    ...
    // NUEVA: Control de explosión
    this.exploded = false;
    this.explosionAmount = 0; // 0 a 1
  ...
  // NUEVA: Función para explosionar
  toggleExplode(e) {
  ...
  // NUEVA: Función para separar componentes
  applyExplosion(x, y, z) {
    const cx = 0, cy = 0, cz = 0; // Centro del modelo
    const dx = x - cx;
    const dy = y - cy;
    const dz = z - cz;
    const dist = Math.sqrt(dx*dx + dy*dy + dz*dz) + 0.1;
    
    // Separar componentes desde el centro
